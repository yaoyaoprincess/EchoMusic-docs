...
| [API 总览 →](./context-api) | 所有人 | `ctx` 对象完整插件 API 速查表 |

### 深入

| 文档 | 内容 |
|------|------|
| [UI 扩展指南 →](./ui-extension) | 页面、设置面板、右键菜单、组件注入、CSS 动态注入 |
| [播放器与音频引擎 →](./player-audio) | 播放控制、播放队列、频谱、歌词系统、自定义音源 |
| [音频频谱 →](./audio-spectrum) | 实时 FFT 频谱数据订阅、可视化、VU 表 |
| [文件存储与数据 →](./filesystem-storage) | 文件系统读写、KV 存储、外观订阅、主题 API |
| [窗口与系统 →](./windows-system) | 浮窗、Now Playing、进程、图标、跨窗口通信 |
