...
| `audioSpectrum` | `ctx.audio.spectrum` — 读取/订阅音频频谱 |
...
| `process` | `ctx.process.launch()` — 启动本机程序 |
...
| 文档 | 适合人群 | 内容 |
|------|:--:|------|
| [快速开始 →](./getting-started) | 新手 | 从零搭建第一个插件 |
| [Manifest 配置参考 →](./manifest) | 所有人 | manifest.json 完整字段说明 |
| [上下文 API 参考 →](./context-api) | 所有人 | `ctx` 对象完整 API 列表 |
| [UI 扩展指南 →](./ui-extension) | 前端开发者 | 页面、设置面板、右键菜单、组件注入 |
| [播放器与音频 →](./player-audio) | 音频处理 | 播放控制、音频频谱、歌词系统 |
| [文件存储与事件 →](./filesystem-events) | 进阶开发 | 本地文件、KV 存储、事件监听、外观订阅 |
| [发布与分发 →](./publishing) | 插件作者 | 插件源、版本管理、打包发布 |
